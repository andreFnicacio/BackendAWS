...
#    region_name='sa-east-1', profile_name='sofie').resource('dynamodb')
...
class AuditTask(object):
    ...
    def __call__(self, **kwargs):
        ...
        """
        # self.__cache_db = Redis(host=os.environ[f'{kwargs["aws"]["stage"]}_REDIS_HOST'],
        #                         port=int(os.environ[f'{kwargs["aws"]["stage"]}_REDIS_PORT']))
        provider = None
        if kwargs["aws"]["stage"] == "PROD":
        ...
        self.__task = dynamodb.Table(
            os.environ[f'{kwargs["aws"]["stage"]}_table_micro_task_in_person']).get_item(Key={'task_id': self.__task_id})['Item']
    ...
    def edit_execution(self, **kwargs):
        ...
        response = dynamodb.Table(
            os.environ[f'{kwargs["aws"]["stage"]}_table_micro_task_execution']).update_item(
            Key={
    ...
    def edit_task(self, **kwargs):
        ...
        dynamodb.Table(
            os.environ[f'{kwargs["aws"]["stage"]}_table_micro_task_in_person']).update_item(
    ...
    def sofier_reward(self, **kwargs):
        ...
        dynamodb.Table(os.environ[f'{kwargs["aws"]["stage"]}_table_sofier_ledger']).update_item(
            Key={
    ...
    def notification(self, **kwargs):
        print('entrei not')
        tb = dynamodb.Table(os.environ[f'{kwargs["aws"]["stage"]}_table_sofier_notification'])
