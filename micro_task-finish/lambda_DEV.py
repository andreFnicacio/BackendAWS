...
def main(event, context):
    ...
    task_info_name = execution_data.get('task_info').get('name')

    if task_info_name == 'marketone':
        fila = sqs.get_queue_by_name(QueueName='queue_mapreduce')

        fila.send_message(
            MessageBody=json.dumps(execution_data, cls=DecimalEncoder),
            MessageAttributes={
                'aws': {
                    'DataType': 'String',
                    'StringValue': "DEV"
