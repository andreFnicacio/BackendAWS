...
def audit(start, finish):
    ...
    df.to_excel(f"/tmp/{data_solicitacao}_task_auditoria.xlsx", index=False, encoding='UTF-8')
    
    path = f"/tmp/{data_solicitacao}_task_auditoria.xlsx"
    name = f"{data_solicitacao}_task_auditoria.xlsx"
    ...
    link = f'https://sofie-reports.s3.sa-east-1.amazonaws.com/{data_solicitacao}_task_auditoria.xlsx'
    print(link)
    return link 
